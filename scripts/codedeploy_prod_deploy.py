...
BUCKET_KEY = os.getenv('AWS_CODEDEPLOY_APP_PROD') + '/' + VERSION_LABEL + \
...
def upload_to_s3(artifact):
    ...
    try:
        client.put_object(
            ...
            Bucket=os.getenv('AWS_S3_BUCKET_PROD'),
            Key=BUCKET_KEY
        )
    except ClientError as err:
...
def deploy_new_revision():
    ...
    """
    try:
        client = boto3.client('codedeploy', region_name='eu-west-3')
    except ClientError as err:
        print("Failed to create boto3 client.\n" + str(err))
        return False
    ...
    try:
        response = client.create_deployment(
            applicationName=str(os.getenv('AWS_CODEDEPLOY_APP_PROD')),
            deploymentGroupName=str(os.getenv('AWS_CODEDEPLOY_GROUP_PROD')),
            revision={
                'revisionType': 'S3',
                's3Location': {
                    'bucket': os.getenv('AWS_S3_BUCKET_PROD'),
